...
export default class CanBeWatchedButton extends AppComponent {
  ...
  updateState() {
    if (
      this.props.slug &&
      this.app.shoppingService.isValidType(this.props.slug)
    ) {
      return this.loadAvailability(this.props.slug)
        ...
        .then(() => {
          ...
          e.classList.toggle(
            's72-show',
            this.state.loaded && this.state.isOwned
          );
        }, utils.noop);
    }
  }
  ...
  render(props, state) {
    if (!state.loaded || !state.isOwned || props.url == '' || props.label == '')
      return;
    return (
      state.loaded &&
      state.isOwned && (
        ...
        >
          <span class="padder" />
          <span class="verb watch s72-btn-can-be-watched-label">
            {props.label}
          </span>
        </button>
      )
